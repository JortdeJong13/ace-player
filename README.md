...
The video element uses native HLS playback, inline playback, AirPlay and the browser's own fullscreen/Picture in Picture controls. Where Safari exposes the Media Session API, the current stream name is also available to system media controls and headset controls. Permanent Safari decode/format errors are left visible with a manual retry so the user can go back and choose another result instead of retrying an incompatible source.
